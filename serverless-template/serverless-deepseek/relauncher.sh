...
while true; do
    ...
    # 启动 vLLM 服务
    LAUNCH_CMD="vllm serve /workspace/deepseek/DeepSeek-R1-Distill-Qwen-1.5B --dtype bfloat16 --port 11434 --max-model-len ${MAX_MODEL_LEN} --gpu-memory-utilization ${GPU_MEMORY_UTILIZATION} --pipeline-parallel-size ${PIPELINE_PARALLEL_SIZE} --max-num-batched-tokens ${MAX_NUM_BATCHED_TOKENS} --max-num-seqs 32"
